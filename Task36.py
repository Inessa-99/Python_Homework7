...
def print_operation_table(num_rows:int, num_columns:int):
    table_row = []
    for i in range(1, num_rows+1):
        table_col = []
        for j in range(1, num_columns+1):
            table_col.append((lambda x, y: x*y)(i, j))
        table_row.append(table_col)
    return table_row

for i in print_operation_table(6,6):
    print("".join(f'{n:5}' for n in i))
